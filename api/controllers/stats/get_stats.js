	...
	switch (period) {
		case "daily": {
			...
			transactions.forEach(item => {
				...
				dbdatakeys[day] = total_amount >= 0 ? total_amount : total_amount * -1;
			});


			console.log(dbdatakeys);
			for (let i = 1; i < 31; i++) {
				let ref = moment().subtract(30-i,'days').format('DD-MM');
				labels.push(ref);
				if(dbdatakeys[ref]){
		...
		case "weekly": {
			transactions = await Transaction.findAll({
				attributes: [
					'createdAt',
					[Sequelize.fn('sum', Sequelize.col('amount')), 'total_amount'],
					Sequelize.literal("CONCAT(YEAR(createdAt), '/', WEEK(createdAt)) as date"),
				],
				where: {
					account_id: client.accounts[0].id,
					amount:{
